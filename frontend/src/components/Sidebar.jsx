  ...
  return (
    <>
      ...
      >
        {/* Logo Area */}
        <div
          className={`h-16 flex items-center justify-between border-b border-white/10 shrink-0 ${
        ...
        {/* New Project Button */}
        <div className={`mt-5 mb-4 ${isCollapsed ? "px-3" : "px-4"}`}>
          <button
            className={`w-full bg-white/10 text-white rounded-lg text-[13px] font-medium hover:bg-white/15 transition-all flex items-center justify-center gap-2 border border-white/10 shadow-sm ${
              isCollapsed ? "py-2 px-0" : "py-2.5 px-3"
            }`}
          >
        ...
        {/* Navigation */}
        <nav className={`flex-1 space-y-1 overflow-y-auto custom-scrollbar ${isCollapsed ? "px-3" : "px-3.5"}`}>
          {navItems.map((item) => {
            const isActive =
        ...
        {/* Bottom Actions */}
        <div className={`mb-2 shrink-0 border-t border-white/10 pt-2 ${isCollapsed ? "px-3" : "px-3.5"}`}>
          <Link
            to="/dashboard"
            className="flex items-center gap-3 px-3 py-2.5 text-[13px] font-medium text-slate-300 hover:text-slate-100 hover:bg-white/5 rounded-lg transition-all"
          ...
          <button 
            onClick={logout}
            className="w-full flex items-center gap-3 px-3 py-2.5 text-[13px] font-medium text-slate-300 hover:text-slate-100 hover:bg-white/5 rounded-lg transition-all cursor-pointer mt-0.5"
